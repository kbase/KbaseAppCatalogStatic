    ...
    var Utils = Object.create({}, {
        ...
        niceElapsedTime: {
            value: function (dateObj, nowDateObj) {
				...
				/**
				* Create an concise description of a time range in relation to the current time.
				* For example 
				*/
        niceElapsedTimeRange: {
            ...
            value: function (from, to, options) {
                ...
                if (from) {
                    if (to) {
                        if (from.getDate() === to.getDate()) {
                            ...
                            return this.niceDate(from, options) + timePart;
                        } else {
                            return 'from ' + this.niceDate(from, options) + ' at ' + this.niceTime(from) + ' to ' + this.niceDate(to, options) + ' at ' + this.niceTime(to);
                        }
                    } else {
                        return 'from ' + this.niceDate(from, options) + ' at ' + this.niceTime(from);
                    }
                }
				...
				niceTimezoneOffset: {
					value: function (date) {
						...
						var unit = offset === 1 ? 'hour' : 'hours';
						var suffix;
						if (offset > 0) {
							suffix = 'earlier than GMT';
						} else if (offset < 0) {
							suffix = 'later than GMT';
						} else {
							'in GMT';
						}
						return offset + ' ' + unit + ' ' + suffix;
					}
				},
        /**
